...
function Delete({ params: specificId }: Id) {
  ...
  const notify = () => {
    if (
      verification.current?.value.toLowerCase() === "yes" &&
      id &&
      alreadyDeleted
    ) {
      return toast("Already deleted ✅", {
        position: "top-right",
        autoClose: 5000,
      ...
      return toast("Delete was Succes ✅", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    <form onSubmit={handleSubmit} className="flex flex-col w-1/2 p-12 mx-auto">
      <div className="p-4 w-fit">
        <p className="font-bold">
          Make sure that you sure want to delete it because it deletes
          permanently
        </p>
      </div>
      <div>
        <input
          ...
          type="text"
          ref={verification}
          placeholder='type "Yes" if you sure want to delete it'
        />
        <button
